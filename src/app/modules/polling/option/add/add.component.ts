...
export class PollingContentAddComponent implements OnInit, AfterViewInit {
  ...
  DataActionAfterAddContentSuccessfulTag(model: PollingContentModel): Promise<any> {
    ...
    return this.pollingContentTagService.ServiceAddBatch(dataListAdd).pipe(
      map(response => {
  ...
  DataActionAfterAddContentSuccessfulOtherInfo(model: PollingContentModel): Promise<any> {
    ...
    return this.pollingContentOtherInfoService.ServiceAddBatch(this.otherInfoDataModel).pipe(
      map(response => {
        if (response.IsSuccess) {
  ...
  DataActionAfterAddContentSuccessfulSimilar(model: PollingContentModel): Promise<any> {
    if (!this.similarDataModel || this.similarDataModel.length === 0) {
      return;
    }
    const dataList: PollingContentSimilarModel[] = [];
    ...
    return this.pollingContentSimilarService.ServiceAddBatch(dataList).pipe(
      map(response => {
        if (response.IsSuccess) {
