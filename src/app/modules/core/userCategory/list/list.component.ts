...
export class CoreUserGroupListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonFaqList(model: CoreUserGroupModel = this.tableRowSelected): void {
    if (!model || !model.Id || model.Id === 0) {
      const title = 'برروز خطا ';
  ...
  onActionbuttonTemplateList(model: CoreUserGroupModel = this.tableRowSelected): void {
    if (!model || !model.Id || model.Id === 0) {
      const title = 'برروز خطا ';
      const message = 'ردیفی برای نمایش انتخاب نشده است';
      this.cmsToastrService.toastr.error(message, title);
